		...
		while (counter < str.length())
		{
			...
			else if ((int)(str.charAt(counter)) <= ' ')
			{
				...
				if(!output.equals(""))
				{
					...
					{
						if (isNumber("" + output.charAt(i))) 
						{
							noNum = false;
							String out = "";
							
							if (i > 0 && Character.isLetter(output.charAt(i-1)))
							{
								result[index] = output.substring(0, i);
								index++;
							}

							while (i < output.length() && isNumber("" + output.charAt(i)))
							{
								out += output.charAt(i);
								i++;
							}
							...
							if (i < output.length())
							{
								result[index] = output.substring(i + 1);
