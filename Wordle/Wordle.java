 ...
 *
 *	@author	Scott DeRuiter and David Greenstein and Tanay Kumar
 *	@version	1.0
...
{ 
	...
	public void initAll(String testWord)
	{
		...
		{
			word = openFileAndChooseWord(WORDS5_ALLOWED, testWord);
		}
	}
	 ...
	 *	THIS METHOD IS INCOMPLETE. Completed by Tanay
	 */
	...
	public String openFileAndChooseWord(String inFileName, String testWord)
	{
		...
		int randomNum = (int)(Math.random()*2309) + 1;
		int count = 0;
		while (reader.hasNext())
		{
			...
			if (count == randomNum && !result.equalsIgnoreCase(testWord))
			{
				result = readLine;
			}
			
			else if (readLine.equalsIgnoreCase(testWord))
			{
				result = testWord;
			} 
		}
	...
	{
		String readLine = "";
		Scanner reader = FileUtils.openToRead("words5allowed.txt");
		
		while (reader.hasNext())
		{
			readLine = reader.nextLine();

			if (readLine.equalsIgnoreCase(possibleWord))
			{
				return true;
			} 
		}
	...
	public void processGuess ( )
	{
        ...
        else 
        {
			...
			JDialog dialog = pane.createDialog(null, "INVALID INPUT");
	...
	public void drawPanel ( )
	{
		...
		{
            int[] currentLetters = {1, 1, 1, 1, 1}; // This color numbers 
												// of the user's word 

            if (wordGuess[row].length() == 5) 
            {
                int counter = 0; // While loop conditonal
                int counter2 = 0; // While loop conditonal
                int index = 0; // Index of a letter in wordleWord
                char letter = ' '; // Letter in the user's guess
                String wordleWord = word; // Wordle word that user must guess.
										// Letters will be removed from 
										// this to avoid duplicates from 
										// being shown as correct 
                
                /* This block finds the letters that need to be green and 
                 * removes those letters form wordleWord.
                 */
                while (counter < 5)
                {
                    if (wordGuess[row].charAt(counter) == word.charAt(counter)) 
                    {
                        currentLetters[counter] = 3;
                        index = wordleWord.indexOf(word.charAt(counter));
                        wordleWord = wordleWord.substring(0,index) +
									wordleWord.substring(index + 1); 
                    }
                    counter++;
                }
                ...
                /* This block finds the letters that need to be yellow and 
                 * removes those letters form wordleWord to avoid duplicates 
                 * from being marked as correct.
                 */
                while (counter < 5) 
                {
					counter2 = 0;
                    letter = wordGuess[row].charAt(counter);

                    while (counter2 < wordleWord.length()) 
                    {
                        if (letter == wordleWord.charAt(counter2) 
							&& currentLetters[counter] != 3) 
                        {
                            currentLetters[counter] = 2;
							wordleWord = wordleWord.substring(0,counter2) +
										wordleWord.substring(counter2 + 1); 
                        }
                        counter2++;
                    }
                    counter++;
                ...
                /* This block checks if the colors for each letter, from 
                 * the array currentLetters, is bigger than the keyBoardColors,
                 * so that biggest color can be show. Also the conditional
                 * checks if the character from Constants.KEYBOARD is equal
                 * to the letter. If the conditionas are true the keyboard
                 * color is changed. This repeats through all 5 letters 
                 * in the user's guess.
                 */ 
                while (counter < 5) 
                {
                    ...
                    while (counter2 < Constants.KEYBOARD.length) 
                    {
                        if (keyBoardColors[counter2] < currentLetters[counter]
							&& Constants.KEYBOARD[counter2].charAt(0) == letter) 
                        {
							keyBoardColors[counter2] = currentLetters[counter];
                        }
            ...
            /* This block is changes the color of the letters using conditionals
             */ 
            for (int col = 0; col < 5; col++) 
            {
                if (wordGuess[row].length() != 0 
					&& currentLetters[col] == 1) 
                {
                    StdDraw.picture(209 + col * 68, 650 - row * 68, 
						"letterFrameDarkGray.png");
                }
                else if (wordGuess[row].length() != 0 
					&& currentLetters[col] == 2) 
                {
                    StdDraw.picture(209 + col * 68, 650 - row * 68, 
						"letterFrameYellow.png");
                }
                else if (wordGuess[row].length() != 0 
					&& currentLetters[col] == 3) 
                {
                    StdDraw.picture(209 + col * 68, 650 - row * 68, 
		...
		/* This block is changes the color of the keyboard using conditionals
		 */ 
		...
		if(readyForMouseInput && StdDraw.isMousePressed())
		{
			for(int i = 0; i < Constants.KEYPLACEMENT.length; i++)
			{
					...
					StdDraw.mouseY() < Constants.KEYPLACEMENT[i][1] + 29)
				{
					...
					else if(activeGame && i != 19 && i != 27 && i != 28 
							&& letters.length() < 5)
					{
